...
補足：
- 投稿本文は一覧カードでは表示せず、投稿詳細モーダルで表示する。
...
- スクロール末尾到達で次ページ取得
- 詳細な seek 条件は、別リポジトリ（バックエンド）の `../footprint/docs/adr/adr_023_seek_pagination_boundary.md` に従う
...
### 処理
- 地図表示範囲変更時は「このエリアで再検索」ボタンを表示する
- 「このエリアで再検索」ボタン押下時に bbox を算出する
...
- 「自分の投稿」「返信履歴」のタブ切り替え
- 自分の投稿一覧（「もっと読み込む」ボタンによる追加取得）
- 自分の返信一覧（「もっと読み込む」ボタンによる追加取得）
...
- レスポンスは配列であり、`page.nextCursor` は返さない
- 投稿一覧・返信一覧は、それぞれ「もっと読み込む」ボタン押下で次ページを取得する
- 詳細な seek 条件は、別リポジトリ（バックエンド）の `../footprint/docs/adr/adr_023_seek_pagination_boundary.md` に従う
...
補足：
- 画面 URL の検索語は `q` で受け取る。
- API 呼び出し時は `q` の値を `keyword` パラメータとして渡す。
